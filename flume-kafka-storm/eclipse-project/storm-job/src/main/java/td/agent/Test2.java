...
//			   bufferWritter.write(lineTxt);
			   ...
			   Object msg_obj = m.get("msg_obj");
			   Object tag = m.get("first_tag");
			   
//			   handle.msg = m.get("msg_obj");
			   
			   // send data to tag proxy service 
		    if(null != tag ){
				   String ClassName = StringUtils.capitalize(tag.toString()) ;
			   try {
				   StreamPlugin this_pares  = (StreamPlugin)Class.forName("td.agent.message.handle."+ClassName).newInstance();
				  // this_pares.doParse((Msg)msg_obj) ;
